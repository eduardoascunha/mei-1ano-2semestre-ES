...
# schemahttps://www.perplexity.ai/discover/
...
async def criar_conta_endpoint(dados: Request_Schema, request: Request):
    if not dados.email or not dados.password or not dados.certificado:
        raise HTTPException(status_code=400, detail="Dados incompletos")
    elif AccountManager.email_existe(dados.email):
        raise HTTPException(status_code=400, detail="Email já registado")
    else:
        ip = request.client.host
        resposta = AccountManager.criar_conta(dados.email, dados.password, dados.certificado)
...
@app.get("/obter_certificado_utilizador/{email_user}")
async def obter_certificado_utilizador(request: Request,email_user:str, token_data:dict=Depends(verificar_access_token)):
    ip = request.client.host
    email_autenticado = token_data.get("sub")
    resposta = DBManager.obter_certificado_utilizador(email_user)
    return resposta
...
async def google_token_exchange(code: str):
    try:
        ...
        if not google_user_id:
            raise HTTPException(status_code=400, detail="ID do Google ou email não encontrado na resposta do Google.")

        email_login_google = user_info.get("email")
        print("email_login_google:", email_login_google)

        # verifica se o user existe
        if not DBManager.existe_email_db(email_login_google):
            raise HTTPException(status_code=400, detail="Email do Google não encontrado na base de dados.")

        # verifica se é o primeiro login, se for atribui o google id
        # se nao pega no user id
        if not DBManager.obter_id_from_google_user_id_db(google_user_id):
            DBManager.inserir_google_user_id_db(email_login_google, google_user_id)

        user_id = DBManager.obter_id_from_email_db(email_login_google)
        email = DBManager.obter_email_por_id_db(user_id)

        DBManager.atualizar_tokens_google_db(google_user_id, google_access_token, google_refresh_token)

        if not email == email_login_google:
            raise HTTPException(status_code=400, detail="Email do Google e email na base de dados não correspondem.")
...
async def backup_google_drive(
    token_data: dict = Depends(verificar_access_token)
):
    email = token_data.get("sub")
    google_access_token = DBManager.obter_access_token_google(email)
    if not google_access_token:
        raise HTTPException(status_code=404, detail="Token de acesso Google não encontrado.")

    ficheiros = DBManager.get_ficheiros_db(email)
    if not ficheiros:
        raise HTTPException(status_code=404, detail="Nenhum ficheiro encontrado na base de dados.")
    ...
    for ficheiro in ficheiros:
        ...
        iv = ficheiro[5]
        tag = ficheiro[6]
        chave_cifrada = DBManager.obter_chave_cifragem(nome, user_id)
        ...
        ficheiro_bytes = json.dumps(ficheiro_backup).encode()
        ...
        multipart = {
            ...
            "file": (nome + ".enc.json", ficheiro_bytes, mime_type)
        }
...
@app.get("/backup/google-drive")
async def backup_google_drive(nome: str, token_data: dict = Depends(verificar_access_token)):
    email = token_data.get("sub")
    google_access_token = DBManager.obter_access_token_google(email)
    if not google_access_token:
        raise HTTPException(status_code=404, detail="Token de acesso Google não encontrado.")

    # procurar ficheiro pelo nome no Google Drive
    ...
    if not files:
        raise HTTPException(status_code=404, detail="Ficheiro não encontrado no Google Drive.")

    file_id = files[0]["id"]

    # receber o conteudo do ficheiro
    download_url = f"https://www.googleapis.com/drive/v3/files/{file_id}?alt=media"
    download_resp = requests.get(download_url, headers=headers)
    if download_resp.status_code != 200:
        raise HTTPException(status_code=download_resp.status_code, detail=download_resp.text)

    # parse ao file json de backup
    try:
        ficheiro_backup = json.loads(download_resp.content)
        # validacao dos campos
        for campo in ["conteudo", "chave_cifrada", "iv", "tag"]:
            if campo not in ficheiro_backup:
                raise ValueError(f"Campo '{campo}' em falta no ficheiro de backup.")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erro ao processar ficheiro: {str(e)}")

    return {
        "status": "sucesso",
        "conteudo": ficheiro_backup["conteudo"],
        "chave_cifrada": ficheiro_backup["chave_cifrada"],
        "iv": ficheiro_backup["iv"],
        "tag": ficheiro_backup["tag"]
    }
