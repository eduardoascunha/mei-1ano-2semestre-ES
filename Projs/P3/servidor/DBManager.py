...
class DBManager:
    ...
    def criar_conta_db(self, email, senha_hash, certificado):
        try:
            cursor = self.conexao.cursor()
            # Criar user
            cursor.execute(
                "INSERT INTO users (email, senha_hash, certificado) VALUES (%s, %s, %s) RETURNING id",
                (email, senha_hash.decode('utf-8'), certificado)
            )
    ...
    def obter_certificado_utilizador(self, email_user):
        try:
            cursor = self.conexao.cursor()
            cursor.execute(
                """
...
######
    ...
    def obter_id_from_google_user_id_db(self, google_user_id):
        cursor = self.conexao.cursor()
    ...
    def obter_google_user_id_from_email_db(self, email):
        cursor = self.conexao.cursor()
        cursor.execute("SELECT google_user_id FROM users WHERE email = %s", (email,))
        resultado = cursor.fetchone()
        cursor.close()
        return resultado[0] if resultado else None

    def atualizar_tokens_google_db(self, google_user_id, google_access_token, google_refresh_token):
        cursor = self.conexao.cursor()
        cursor.execute(
            "UPDATE users SET google_access_token = %s, google_refresh_token = %s WHERE google_user_id = %s",
            (google_access_token, google_refresh_token, google_user_id)
        )
        self.conexao.commit()
        return True

    def obter_access_token_google(self, email):
        cursor = self.conexao.cursor()
        cursor.execute("SELECT google_access_token FROM users WHERE email = %s", (email,))
        resultado = cursor.fetchone()
        cursor.close()
        return resultado[0] if resultado else None

    def get_ficheiros_db(self, email):
        user_id = self.obter_id_from_email_db(email)
        cofre_id = self.obter_cofreID_db(user_id)
        cursor = self.conexao.cursor()
        cursor.execute("SELECT * FROM ficheiros WHERE cofre_id = %s", (cofre_id,))
        resultado = cursor.fetchall()
        cursor.close()
        return resultado

    def inserir_google_user_id_db(self, email, google_user_id):
        cursor = self.conexao.cursor()
        cursor.execute(
            "UPDATE users SET google_user_id = %s WHERE email = %s",
    ...
    def obter_chave_cifragem(self, nome, user_id):
        cofre_id = self.obter_cofreID_db(user_id)
        ficheiro_id = self.obter_id_ficheiro_por_nome_e_parent(nome, None, cofre_id)

        cursor = self.conexao.cursor()
        cursor.execute("SELECT chave_cifrada FROM chaves WHERE user_id = %s AND ficheiro_id = %s", (user_id, ficheiro_id))
        resultado = cursor.fetchone()
        cursor.close()
        return resultado[0] if resultado else None